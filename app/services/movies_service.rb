class MoviesService
  ...
  def general_movie_details(movie_id)
    results = conn.get("https://api.themoviedb.org/3/movie/#{movie_id}?api_key=#{ENV['MOVIES_API_KEY']}")
    JSON.parse(results.body, symbolize_names: true)
  end

  def movie_reviews(movie_id)
    results = conn.get("https://api.themoviedb.org/3/movie/#{movie_id}/reviews?api_key=#{ENV['MOVIES_API_KEY']}")
    JSON.parse(results.body, symbolize_names: true)
  end

  def movie_cast(movie_id)
    results = conn.get("https://api.themoviedb.org/3/movie/#{movie_id}/credits?api_key=#{ENV['MOVIES_API_KEY']}")
    JSON.parse(results.body, symbolize_names: true)
  end
